(define (createcylinder name radius height)
  ...
  (define (getarea) 
    (+ (* 2 (getpi) (getradius) (getheight)) (* 2 (getpi) (* (getradius) (getradius))))
  )
  (define (getvolume)
    (* (getpi) (* (getradius) (getradius)) (getheight))
  )
  (define (comparisonOfTypes op value)
    (cond 
      ((string=? op "==")
        (if (string=? "cylinder" value) #t #f)
      )
      ((string=? op "!=")
        (if (not (string=? "cylinder" value)) #t #f)
      )
      ((string=? op ">=")
        (if (string>=? "cylinder" value) #t #f)
      )
  ...
  (define (getinfo)
    (map display (list "Cylinder: " (getname) ", Radius=" (getradius) ", Height=" (getheight) "\n"
